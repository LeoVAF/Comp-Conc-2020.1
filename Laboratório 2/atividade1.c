...
int main(){
	...
	int thread[NTHREADS];
	// Alocação de memória (a função malloc() não estava funcionando corretamente: ao escrever em C, havia escrita em alguns elementos da matriz A ou B)
	int tam = N*N;
	double x[tam],y[tam],z[tam];
	A = x;
	B = y;
	C = z;
